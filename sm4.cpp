...
void sm4_aRound(uint32_t& x0, uint32_t& x1, uint32_t& x2, uint32_t& x3, uint32_t k) {
    uint32_t temp = x0 ^ L_transform0(tau(x1 ^ x2 ^ x3 ^ k));
    x0 = x1;
    x1 = x2;
    x2 = x3;
    x3 = temp;
...
void sm4_encrypt_block(uint32_t* key, uint8_t* plaintext, uint8_t* ciphertext) {
    uint32_t input[4];
    for (int j = 0; j < 4; j++) {
    ...
    for (int i = 0; i < 32; i++) {
        sm4_aRound(input[0], input[1], input[2], input[3], key[i + 4]);
    }
...
int main() {
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        sm4_encrypt_block(key, plaintext, ciphertext);
