...
| **NAF编码优化**        | _naf()方法将标量转为非相邻形式，_point_mul_naf()实现NAF点乘           | 减少约33%的点加次数，256位标量乘法平均从127次点加降至85次                        |
| **固定基点预计算**      |_build_precompute_table()预计算[2⁰G,2¹G,...,2²⁵⁵G]的仿射坐标            | 密钥生成从255次点加倍+127次点加优化为255次查表+约16次点加（基于汉明重量）            |
| **验签点加合并**        | sm2_verify()中合并计算sG+tP，共享中间变量                               | 验签运算从2组独立点乘(510D+254A)优化为255D+170A，节约约30%计算量                   |
