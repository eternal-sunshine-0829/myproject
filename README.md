...
| 优化措施       | 说明                                                                           | 性能提升效果 |
|----------------|-------------------------------------------------------------------------------|------------------|
| T-table优化   | 将S盒变换与循环移位合并，定义四个 8bit→32bit 查找表                             | 减少实时运算，提升加密速度 |
| AESNI优化     | 用SIMD并行处理多个分组数据，并用AESENCLAST实现S盒变换（借助有限域同构映射）   | 在硬件中一次性完成多个步骤，减少指令数和内存访问 |

- sm4未优化，使用T-table优化sm4，使用T-table优化sm4三者测试得到时间如下图，观察到使用T-table优化sm4使得性能提升约20%；但使用AESNI优化sm4却导致性能下降，推测其原因为：本实验测试数据规模较小，数据准备、映射转换和 SIMD启动开销占据主要时间，从而导致性能下降。
  
  <img width="640" height="345" alt="image" src="https://github.com/user-attachments/assets/1fdb0677-64f7-4c34-bbf8-904a4e5b7fa9" />



#### b): 基于SM4的实现，做SM4-GCM工作模式的软件优化实现 （❌ 未完成）<br><br>
...
| **频域选择**      | `get_lowfreq_positions()`   | 在DCT变换域的低频系数嵌入水印，提高对常规图像处理的抵抗能力  | JPEG压缩、滤波操作         |
| **冗余嵌入**      | `repeat`参数（默认5）                | 每个编码比特在多个位置重复嵌入，使用多数表决机制提高提取准确性              | 局部裁剪、涂改攻击         |
| **几何校正**      | `align_image_by_features()` | 采用ORB特征检测+RANSAC单应矩阵计算实现图像自动对齐                          | 旋转、缩放、平移等几何攻击 |
| **通道选择**      | Y通道嵌入                   | 在YCrCb色彩空间的亮度通道(Y)嵌入，提高视觉不可见性和稳定性       | 色彩调整、色度压缩         |
...
| 优化措施               | 说明                                                                 | 性能提升效果                                                                 |
|-----------------------|-------------------------------------------------------------------------|----------------------------------------------------------------------------|
| **雅可比坐标系统**      | 通过`_affine_to_jacobian()`和`_jacobian_to_affine()`实现坐标转换          | 将点加运算中的模逆转换为乘法，雅可比坐标下点加保持12M+4S+7A但实际运行更快           |
| **NAF编码优化**        | `_naf()`方法将标量转为非相邻形式，`_point_mul_naf()`实现NAF点乘           | 256位标量乘法平均从127次点加降至85次                        |
| **固定基点预计算**      |`_build_precompute_table()`预计算`[2⁰G,2¹G,...,2²⁵⁵G]`的仿射坐标            | 密钥生成从255次点加倍+127次点加优化为255次查表+约16次点加     |
